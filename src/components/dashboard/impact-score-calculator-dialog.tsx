...
export function ImpactScoreCalculatorDialog({ children }: { children: ReactNode }) {
  ...
  const [stage, setStage] = useState<'loadingQuestions' | 'form' | 'calculating' | 'result'>('form');
  ...
  useEffect(() => {
    if (open) {
      ...
      if (user?.lastImpactScoreCalc) {
        const lastCalcDate = new Date(user.lastImpactScoreCalc);
        const twentyFourHoursLater = addHours(lastCalcDate, 24);
        if (new Date() < twentyFourHoursLater) {
          onCooldown = true;
          setNextAvailableTime(formatDistanceToNow(twentyFourHoursLater, { addSuffix: true }));
        }
      }
      setIsOnCooldown(onCooldown);

      if (!onCooldown && questions.length === 0) {
        fetchQuestions();
      }
    }
  }, [open, user?.lastImpactScoreCalc, questions.length]);
    ...
    if (isOnCooldown) {
      return (
        <div className="flex flex-col items-center justify-center p-12 text-center h-64">
          ...
          <p className="text-lg font-semibold text-foreground">You can calculate your score again</p>
          <p className="text-2xl font-bold text-accent">{nextAvailableTime}</p>
          <p className="mt-4 text-sm text-muted-foreground">Come back later to update your impact score and earn more points!</p>
        </div>
      );
    }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isOnCooldown 
              ? "You can calculate your score once every 24 hours." 
